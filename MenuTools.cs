...
namespace BagOfTricks
{
    public static class MenuTools
    {
        ...
        public static void ToggleButtonDependant(ref string toggle, ref string dependancy, string buttonText,
            string buttonTooltip)
        {
            ...
            if (GL.Button(TextWithTooltip(buttonText, buttonTooltip, $"{toggle}" + " "), GL.ExpandWidth(false)))
            {
                ...
                {
                    if (!Strings.ToBool(dependancy)) dependancy = Storage.isTrueString;
        ...
        public static void ToggleButtonActionAtOn(ref string toggle, Action action, string buttonText,
            string buttonTooltip)
        {
            GL.BeginHorizontal();
            if (GL.Button(TextWithTooltip(buttonText, buttonTooltip, $"{toggle}" + " "), GL.ExpandWidth(false)))
        ...
        public static void ToggleButtonActionAtOff(ref string toggle, Action action, string buttonText,
            string buttonTooltip)
        {
            GL.BeginHorizontal();
            if (GL.Button(TextWithTooltip(buttonText, buttonTooltip, $"{toggle}" + " "), GL.ExpandWidth(false)))
        ...
        public static void ToggleButtonFavouritesMenu(ref string toggle, string buttonText, string buttonTooltip,
            bool boldText = false)
        {
            GL.BeginHorizontal();
            if (GL.Button(TextWithTooltip(buttonText, buttonTooltip, $"{toggle}" + " ", "", boldText),
                GL.ExpandWidth(false)))
            {
                if (toggle == Storage.isFalseString)
                    toggle = Storage.isTrueString;
                else if (toggle == Storage.isTrueString) toggle = Storage.isFalseString;
            }
        }

        public static void ToggleButton(ref string toggle, string buttonText, string buttonTooltip,
            bool boldText = false)
        {
            GL.BeginHorizontal();
            if (GL.Button(TextWithTooltip(buttonText, buttonTooltip, $"{toggle}" + " ", "", boldText),
        ...
        public static void ToggleButton(ref string toggle, string buttonText, string buttonTooltip, string toggleName,
            bool boldText = false)
        {
        ...
        public static GUIContent TextWithTooltip(string textMain, string tooltip, string textFront = "",
            string textBack = "", bool boldText = false)
        {
            if (boldText)
                return new GUIContent(textFront + RichText.Bold(Strings.GetText(textMain)) + textBack,
                    Strings.GetText(tooltip));
        ...
        {
            var content = Strings.GetText(label);
            if (bold) content = RichText.Bold(content);
        ...
        public static void ToggleToToggleButton(ref string toggle)
        {
            if (toggle == "false")
                ...
                toggle = Storage.isTrueString;
            else if (toggle != Storage.isFalseString && toggle != Storage.isTrueString) toggle = Storage.isFalseString;
        }
        ...
        public static void Tooltips()
        {
            ...
            var rectX = Input.mousePosition.x - Storage.ummRect.min.x + Storage.ummScrollPosition[Storage.ummTabId].x -
                        8;
            if (rectX > 470) rectX = rectX - tooltipSize.x + 8;
            var rectY = Screen.height - Storage.ummRect.min.y + Storage.ummScrollPosition[Storage.ummTabId].y -
                        Input.mousePosition.y - 65 - textHeight;
                ...
                switch (toggleButton)
                {
                    ...
                    default:
                        throw new ArgumentException(
                            $"GetToggleButton received an invalid toggle name ({toggleButton})!");
                }
            }
            catch (Exception e)
            {
        ...
        public static void AddUpDownButtons(string methodName, ref string[] methods, int size)
        {
            if (Array.IndexOf(methods, methodName) > 0)
                if (GL.Button(new GUIContent(RichText.Size(Storage.upArrow, size), Strings.GetText("tooltip_ArrowUp")),
                    GL.ExpandWidth(false)))
                {
            ...
            if (Array.IndexOf(methods, methodName) < methods.Length - 1)
                if (GL.Button(
                    new GUIContent(RichText.Size(Storage.downArrow, size), Strings.GetText("tooltip_ArrowDown")),
                    GL.ExpandWidth(false)))
                {
                    if (Input.GetKey(KeyCode.LeftShift) || Input.GetKeyDown(KeyCode.RightShift))
        ...
        {
            if (float.TryParse(stringSetting, out var result))
                finalSetting = result;
            else
                stringSetting = "";
        }
        ...
        {
            if (int.TryParse(setting, out var iTest) && Regex.IsMatch(setting, @"^[0]*$") == false)
                return setting;
            return "";
        }
        ...
        {
            if (float.TryParse(setting, out var fTest) && !setting.Contains(","))
                return setting;
            return "";
        }
        ...
        public static void CreateStatInterface(string stat, CharacterStats charStats, StatType statType, int amount,
            bool isSkill = false)
        {
            GL.BeginVertical("box");
            GL.BeginHorizontal();
            GL.Label($"{stat}: {charStats.GetStat(statType).BaseValue} ({charStats.GetStat(statType).ModifiedValue})");
            if (GL.Button("<b>+</b>")) charStats.GetStat(statType).BaseValue++;
            if (GL.Button("<b>-</b>")) charStats.GetStat(statType).BaseValue--;
            ...
            if (isSkill)
            {
                var modSkill = charStats.GetStat(statType) as ModifiableValueSkill;
        ...
        public static void CreateStatMultiplierInterface(string stat, CharacterStats charStats, StatType statType,
            List<UnitEntityData> units, float multiplier)
        {
            GL.BeginVertical("box");
            GL.BeginHorizontal();
            GL.Label(stat + " ");
            if (GL.Button(Strings.GetText("button_Apply") + $" (X{Math.Round(multiplier, 1)})", GL.ExpandWidth(false)))
            {
                if (multiplier != 1f)
                    foreach (var unit in units)
                    {
                        ...
                        stats.GetStat(statType).BaseValue =
                            Mathf.RoundToInt(stats.GetStat(statType).BaseValue * (float) Math.Round(multiplier, 1));
                    }
        ...
        public static void UnitAlignment(UnitEntityData unit)
        {
            ...
            GL.BeginHorizontal();
            Storage.statsSelectedAlignmentIndex =
                GL.SelectionGrid(Storage.statsSelectedAlignmentIndex, Storage.alignmentsArrayStats, 3);
            GL.EndHorizontal();
            GL.Space(10);
            ...
            {
                if (GL.Button(
                    TextWithTooltip("button_SetAlignment", "tooltip_SetAlignment", "",
                        $" {Storage.alignmentsArrayStats[Storage.statsSelectedAlignmentIndex]}"), GL.ExpandWidth(false))
                )
                    unit.Descriptor.Alignment.Set(
                        (Alignment) Common.IndexToAligment(Storage.statsSelectedAlignmentIndex));
            }
            else
            {
                if (GL.Button(
                    TextWithTooltip("button_AlignmentCantBeSetTo", "tooltip_SetAlignment", "",
                        $"<b> {Storage.alignmentsArrayStats[Storage.statsSelectedAlignmentIndex]} {Strings.GetText("button_AlignmentUseShiftInstead")}</b>",
            ...
            GL.BeginHorizontal();
            if (GL.Button(
                TextWithTooltip("button_ShiftAlignmentTowards", "tooltip_ShiftAlignmentTowards", "",
                    $" {Storage.alignmentsArrayStats[Storage.statsSelectedAlignmentIndex]} {Strings.GetText("misc_By")} {unitAlignmentTextField.finalAmount}"),
                GL.ExpandWidth(false)))
                unit.Descriptor.Alignment.Shift((AlignmentShiftDirection) Storage.statsSelectedAlignmentIndex,
                    unitAlignmentTextField.finalAmount, null);
            GL.EndHorizontal();
            GL.Space(10);
            SingleLineLabel(Strings.GetText("label_CurrentAlignment") + ": " +
                            Common.AlignmentToString(unit.Descriptor.Alignment.Value));
            GL.EndVertical();
        }
        ...
        public static void AddSingleItemAmount(string itemGuid, int itemAmount, bool identify = false)
        {
            ...
            if (settings.itemGuid != "c5d4962385e0e9c439ab935d83361947")
            {
                ...
                {
                    var itemEntity = Utilities.GetBlueprintByGuid<BlueprintItem>(itemGuid).CreateEntity();
                    itemEntity.Identify();

                    for (var i = 0; i < itemAmount; i++) Game.Instance.Player.Inventory.Add(itemEntity);
                }
            }
            else
            {
        ...
        {
            var label = keyCode == KeyCode.None ? Strings.GetText("button_PressKey") : keyCode.ToString();
            if (GL.Button(label, GL.ExpandWidth(false))) keyCode = KeyCode.None;
    ...
    public static class Strings
    {
        ...
        public static string SpaceBeforeCaptialFormat(string s)
        {
            ...
            {
                if (char.IsUpper(s[i]) && s[i - 1] != ' ') stringBuilder.Append(' ');
        ...
        public static Dictionary<string, string> XMLtoDict(string loc)
        {
            try
            {
                ...
                var serializer = new XmlSerializer(typeof(Localisation[]),
                    new XmlRootAttribute {ElementName = "resources"});
                using (var stream =
                    File.OpenRead(Storage.modEntryPath + Storage.localisationFolder + "\\" + loc + ".xml"))
                {
        ...
        public static void RefreshStrings()
        {
            ...
            Storage.individualSkillsArray = new[]
            {
                GetText("charStat_Athletics"), GetText("charStat_KnowledgeArcana"), GetText("charStat_KnowledgeWorld"),
                GetText("charStat_LoreNature"), GetText("charStat_LoreReligion"), GetText("charStat_Mobility"),
                GetText("charStat_Perception"), GetText("charStat_Persuasion"), GetText("charStat_Stealth"),
                GetText("charStat_Thievery"), GetText("charStat_UseMagicDevice"), GetText("charStat_Bluff"),
                GetText("charStat_Diplomacy"), GetText("charStat_Intimidate")
            };
            Storage.individualSavesArray = new[]
                {GetText("charStat_Fortitude"), GetText("charStat_Reflex"), GetText("charStat_Will")};
            ...
            Storage.alignmentsArrayKingdom = new[]
            {
                GetText("arrayItem_Alignments_LawfulGood"), GetText("arrayItem_Alignments_NeutralGood"),
                GetText("arrayItem_Alignments_ChaoticGood"), GetText("arrayItem_Alignments_LawfulNeutral"),
                GetText("arrayItem_Alignments_TrueNeutral"), GetText("arrayItem_Alignments_ChaoticNeutral"),
                GetText("arrayItem_Alignments_LawfulEvil"), GetText("arrayItem_Alignments_NeutralEvil"),
                GetText("arrayItem_Alignments_ChaoticEvil"), GetText("arrayItem_Alignments_NoChange")
            };
            Storage.alignmentsArrayStats = new[]
            {
                GetText("arrayItem_Alignments_LawfulGood"), GetText("arrayItem_Alignments_NeutralGood"),
                GetText("arrayItem_Alignments_ChaoticGood"), GetText("arrayItem_Alignments_LawfulNeutral"),
                GetText("arrayItem_Alignments_TrueNeutral"), GetText("arrayItem_Alignments_ChaoticNeutral"),
                GetText("arrayItem_Alignments_LawfulEvil"), GetText("arrayItem_Alignments_NeutralEvil"),
                GetText("arrayItem_Alignments_ChaoticEvil"), GetText("arrayItem_Alignments_Good"),
                GetText("arrayItem_Alignments_Evil"), GetText("arrayItem_Alignments_Lawful"),
                GetText("arrayItem_Alignments_Chaotic")
            };
            Storage.alignmentsArrayKingdomStats = new[]
            {
                GetText("arrayItem_Alignments_LawfulGood"), GetText("arrayItem_Alignments_NeutralGood"),
                GetText("arrayItem_Alignments_ChaoticGood"), GetText("arrayItem_Alignments_LawfulNeutral"),
                GetText("arrayItem_Alignments_TrueNeutral"), GetText("arrayItem_Alignments_ChaoticNeutral"),
                GetText("arrayItem_Alignments_LawfulEvil"), GetText("arrayItem_Alignments_NeutralEvil"),
                GetText("arrayItem_Alignments_ChaoticEvil")
            };
            Storage.takeXArray = new[]
            {
                GetText("arrayItem_takeX_Off"), GetText("arrayItem_takeX_Take10"), GetText("arrayItem_takeX_Take20"),
                GetText("arrayItem_takeX_TakeCustomValue")
            };
            Storage.charSizeArray = new[]
            {
                GetText("arrayItem_Size_Fine"), GetText("arrayItem_Size_Diminutive"), GetText("arrayItem_Size_Tiny"),
                GetText("arrayItem_Size_Small"), GetText("arrayItem_Size_Medium"), GetText("arrayItem_Size_Large"),
                GetText("arrayItem_Size_Huge"), GetText("arrayItem_Size_Gargantuan"), GetText("arrayItem_Size_Colossal")
            };
            Storage.featsParamArray = new[]
            {
                GetText("misc_None"), GetText("arrayItem_FeatParam_FencingGrace"),
                GetText("arrayItem_FeatParam_ImprovedCritical"), GetText("arrayItem_FeatParam_SlashingGrace"),
                GetText("arrayItem_FeatParam_SwordSaintChosenWeapon"), GetText("arrayItem_FeatParam_WeaponFocus"),
                GetText("arrayItem_FeatParam_WeaponFocusGreater"), GetText("arrayItem_FeatParam_WeaponMastery"),
                GetText("arrayItem_FeatParam_WeaponSpecialization"),
                GetText("arrayItem_FeatParam_WeaponSpecializationGreater"), GetText("arrayItem_FeatParam_SpellFocus"),
                GetText("arrayItem_FeatParam_GreaterSpellFocus")
            };
            Storage.neverRollXArray = new[]
            {
                GetText("arrayItem_NeverRollX_Everyone"), GetText("arrayItem_NeverRollX_OnlyParty"),
                GetText("arrayItem_NeverRollX_OnlyEnemies")
            };
            Storage.unitEntityDataSelectionGridArray = new[]
            {
                GetText("arrayItem_NeverRollX_Everyone"), GetText("arrayItem_NeverRollX_OnlyParty"),
                GetText("arrayItem_NeverRollX_OnlyMainChar"), GetText("arrayItem_NeverRollX_OnlyEnemies")
            };
            Storage.unitEntityDataArray = new[]
            {
                GetText("arrayItem_UnityEntityData_Party"), GetText("arrayItem_UnityEntityData_ControllableCharacters"),
                GetText("arrayItem_UnityEntityData_ActiveCompanions"),
            ...
            Storage.unitEntityDataArrayNoEnemies =
                Storage.unitEntityDataArray.Take(Storage.unitEntityDataArray.Count() - 1).ToArray();
            Storage.encumbranceArray = new[]
            {
                GetText("encumbrance_Light"), GetText("encumbrance_Medium"), GetText("encumbrance_Heavy"),
                GetText("encumbrance_Overload")
            };
            Storage.inventoryItemTypesArray = new[]
            {
                RichText.Bold(GetText("misc_All")), GetText("label_Armours"), GetText("label_Belts"),
                GetText("label_Footwear"), GetText("label_Gloves"), GetText("label_Headwear"),
                GetText("label_Neckwear"), GetText("label_NonUsable"), GetText("misc_Other"), GetText("label_Rings"),
                GetText("label_Shields"), GetText("label_ShoulderItems"), GetText("label_UsableItems"),
                GetText("label_Weapons"), GetText("label_WristItems")
            };
            Storage.weatherArray = new[]
            {
                GetText("arrayItem_Weather_Normal"), GetText("arrayItem_Weather_Rain"),
                GetText("arrayItem_Weather_Snow")
            };
        }
    ...
    {
        public int indexUnitEntitiyData;
        public int indexUnitEntitiyDataOld;
        private readonly Player player = Game.Instance.Player;
        ...
        private void UpdateUnityEntityData()
        {
            if (unitEntityData.Count != unitEntityDataOld.Count || Storage.reloadPartyBuffs)
            {
                ...
                foreach (var controllableCharacter in unitEntityData)
        ...
        {
            if (unitEntityData.Count - 1 < indexUnitEntitiyData) indexUnitEntitiyData = unitEntityData.Count - 1;
        }
    }
